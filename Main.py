...
def run_slam_comparison_simultaneous():
    ...
    res_noda = (true_history_noda, path_history_noda, x_error_noda, y_error_noda, th_error_noda, map_list_noda, x_ukf_noda, P_noda, true_landmarks)
    res_da = (true_history_da, path_history_da, x_error_da, y_error_da, th_error_da, map_list_da, x_ukf_da, P_da, true_landmarks)
...
def plot_pose_errors(res, title_suffix):
    _, _, x_error, y_error, th_error, _, _, _, _ = res
...
def plot_landmark_errors(res, title_suffix, is_da):
    _, _, _, _, _, map_list, x_ukf, _, true_landmarks = res
...
def plot_comparison_map(res_da, res_noda):
    true_history, path_da, _, _, _, _, _, _, true_landmarks = res_da
...
def plot_comparison_pose_errors(res_da, res_noda):
    _, _, err_x_da, err_y_da, err_th_da, _, _, _, _ = res_da
    _, _, err_x_noda, err_y_noda, err_th_noda, _, _, _, _ = res_noda
...
def plot_comparison_landmark_errors(res_da, res_noda):
    _, _, _, _, _, map_list_da, x_ukf_da, _, true_lm_da = res_da
    _, _, _, _, _, map_list_noda, x_ukf_noda, _, true_lm_noda = res_noda
